...
def create_callback_fn(callback_citation_key: str = "callback_citations", chat_completion_fn: Callable[[str, List[Dict], dict], str] = None):
    ### create callback_fn with customized chat_completion function. 

    async def sim_callback(question, conversation_history, meta_data):
        # get context_key from cov_template
        context_key = conv_template.context_key[0]

        # get messages from conversation_history and convert into openai chat format
        messages = []
        ...
        # parse the context generated by callback function and save to 
        # meta_data context.
